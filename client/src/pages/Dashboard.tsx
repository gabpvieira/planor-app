...
export default function Dashboard() {
  ...
  const todaysAppointments = appointments?.slice(0, 3) || []; // In real app, filter by date
    ...
    <div className="p-8 max-w-7xl mx-auto space-y-8">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">{formattedDate} — Here is what's happening today.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {/* Daily Insight Card */}
        <motion.div 
          className="col-span-1 md:col-span-2 lg:col-span-3 p-6 rounded-2xl bg-gradient-to-r from-primary/90 to-blue-600 text-white shadow-lg shadow-primary/20"
          ...
          <CardContent className="space-y-4">
             {todaysAppointments.length === 0 ? (
               ...
               todaysAppointments.map((apt) => (
                  <div key={apt.id} className="flex items-center gap-3 p-3 rounded-lg bg-muted/40 border border-transparent hover:border-border transition-colors">
                     <div className="h-10 w-1 rounded bg-primary/40 shrink-0" />
                     <div>
          ...
          <CardContent className="space-y-4">
            {todaysHabits.length === 0 ? (
               <div className="flex flex-col items-center justify-center py-8 text-center text-muted-foreground">
                  ...
                  <p className="text-sm">Start a new habit today.</p>
               </div>
            ) : (
